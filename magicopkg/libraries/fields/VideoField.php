...
class VideoField extends Textbox {
	...
	private $videoField, $videoDurationField;
	...
	function postSetParent() {
		...
		$this->videoField = strtolower( $this->label );
		$this->videoDurationField = strtolower("{$this->label}_duration");
	}

	function setDatabaseFields() {
		$this->databaseFields = array (
			$this->videoField => array(
				'type' => 'VARCHAR',
				'constraint' => 200
			),
			$this->videoDurationField => array(
				'type' => 'INT'
			)
		);
	}
	...
	function save($table, $id) {
		...
		if ( $_POST[$this->name] )
		{
			...
			$ci->db->where('id', $id);
			$ci->db->update($table, array($this->videoField => $_POST[$this->name], $this->videoDurationField => $this->getVideoDuration($_POST[$this->name])));
		}
		else
		{
			$ci->db->where('id', $id);
			$ci->db->update($table, array($this->videoField => '', $this->videoDurationField => ''));
		}
	}
	...
	function setFieldValue($table, $id, $row) {
		$this->value = $row[$this->videoField];
	}
